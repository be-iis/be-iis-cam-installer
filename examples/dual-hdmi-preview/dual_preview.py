...
CAPTURE_WIDTH, CAPTURE_HEIGHT = 1024, 576
FRAMERATE = 30
...
def branch(name, pad):
    return (
        f"appsrc name={name} is-live=true block=true do-timestamp=true format=time ! "
        f"rawvideoparse format=i420 width={CAPTURE_WIDTH} height={CAPTURE_HEIGHT} "
        f"framerate={FRAMERATE}/1 ! videoconvert ! videoscale ! "
        f"video/x-raw,width={PREVIEW_WIDTH},height={PREVIEW_HEIGHT},pixel-aspect-ratio=1/1 ! "
...
def main():
    Gst.init(None)
    description = " ".join((
        branch("camera0", "sink_0"), branch("camera1", "sink_1"),
        "compositor name=compositor "
        f"sink_0::xpos=0 sink_0::ypos={PREVIEW_Y} "
        f"sink_1::xpos={PREVIEW_WIDTH} sink_1::ypos={PREVIEW_Y} ! "
        f"video/x-raw,width={DISPLAY_WIDTH},height={DISPLAY_HEIGHT},pixel-aspect-ratio=1/1 ! "
    ...
    for process, name in zip(captures, ("camera0", "camera1")):
        threading.Thread(target=feed, args=(process, pipeline.get_by_name(name)), daemon=True).start()
    try:
        loop.run()
    finally:
