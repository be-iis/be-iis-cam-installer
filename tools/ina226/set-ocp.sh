...
configure_ocp(){
 local label="$1" address="$2" limit_ma="$3" auto_release="$4"
 local shunt_uv limit_counts mask
 shunt_uv=$(( limit_ma * INA226_SHUNT_MOHM ))
 limit_counts=$(( (shunt_uv * 2 + 2) / 5 ))
 (( limit_counts > 0 && limit_counts <= 0x7fff )) || ina226_die "${label}: limit outside INA226 range."
 ...
 printf '%s: OCP=%d mA, shunt=%d mOhm, limit=0x%04x, mode=%s\n' "$label" "$limit_ma" "$INA226_SHUNT_MOHM" "$limit_counts" "$([[ "$auto_release" -eq 1 ]] && printf auto-release || printf latched)"
}
main(){ ina226_need_root; ina226_prepare_bus; if ! ina226_parse_common_arguments 400 "$@"; then usage; exit 0; fi; ina226_for_selected_links "$INA226_SELECTED_LINKS" configure_ocp "$INA226_REQUESTED_LIMIT" "$INA226_AUTO_RELEASE"; }
main "$@"
